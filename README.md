...
| **Only send latest board to AI** | On = only the newest `<info_board>` is included when building the prompt sent to the model, keeping that prompt shorter. Off = every old board still present in the chat history is included too. **Default: off.** |
